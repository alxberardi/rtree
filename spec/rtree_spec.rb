...
describe RTree::Tree do
  ...
  subject(:root) { RTree::Tree.new('root') }

  context 'upon creation with content' do
    it { should have_content }
    its(:content) { should == 'root' }
  end

  context 'upon creation without content' do
    subject(:root) { RTree::Tree.new }

    it { should_not have_content }
    its(:content) { should be_nil }
  end

  context 'after creation of the root node' do
  ...
  context 'after creating a tree with children' do
    subject(:root) { TreeBuilder.build(3, 2) }
  ...
  context 'given an internal node' do
    subject(:root) { TreeBuilder.build(3, 2) }
    let(:node) { root['root_child_1_child_0'] }
  ...
  context 'given a leaf node' do
    subject(:root) { TreeBuilder.build(3, 2) }
    let(:node) { root['root_child_1_child_0_child_1'] }
  ...
  context 'when visiting the nodes' do
    subject(:root) { TreeBuilder.build(2, 2) }
    let(:depth) { ['root',
                   'root_child_0',
                   'root_child_0_child_0',
                   'root_child_0_child_1',
  ...
  context 'when searching for nodes' do
    subject(:root) { TreeBuilder.build(2, 2) }
    ...
    it 'should allow searching in depth for all nodes that match a condition' do
      ...
      found_nodes.should_not be_empty
      found_nodes.map(&:content).should eql ['root_child_0',
                                             'root_child_0_child_0',
                                             'root_child_0_child_1',
                                             'root_child_1',
                                             'root_child_1_child_0',
  ...
  context 'when implementing a Tree class' do
    before do
      class ValidNode < RTree::Tree;
      end
      class InvalidNode;
      end
      class ExtendedValidNodeOne < ValidNode;
      end
      class ExtendedValidNodeTwo < ValidNode;
      end
    end
    ...
    it 'when extending a node class it should inherit valid child node classes definitions' do
      ...
      class ExtendedNodeClass < BaseNodeClass;
      end
    ...
    it 'should allow identifying leaf node classes' do
      ...
      class InternalNodeClass < ValidNode;
      end
  ...
  context 'when adding ordered nodes' do
    subject(:root) { RTree::Tree.new('root') }
